...
import { ImageIcon, X, Grid, List, Edit } from "lucide-react"
...
export function MediaSelector({
  ...
  const handleUpdateComplete = (updatedFile: MediaFile) => {
    // Update the file in the selected files list
    setSelectedFiles(prev => 
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent className="min-w-6xl min-h-[80vh]">
          <DialogHeader>
            ...
            <TabsContent value="existing" className="space-y-4">
              ...
              {selectedFiles.length > 0 && (
                <div className="border rounded-lg px-3 py-2 space-y-2">
                  <h4 className="font-medium">Selected File{selectedFiles.length !== 1 ? 's' : ''}</h4>
                  <div className="space-y-2">
                    {selectedFiles.map((file) => (
                      <div key={file.id} className="flex flex-col items-start w-full justify-between gap-2">
                        ...
                        <div className="flex flex-col">
                          <span className="text-xs text-muted-foreground">{file.alt || ''}</span>
                          <span className="text-xs text-muted-foreground">{file.caption || ''}</span>
                        </div>
                      </div>
                    ))}
                  </div>
